...
/obj/effect/proc_holder/cultmark/InterceptClickOn(mob/living/the_one_who_called, params, atom/target)
	if(..())
		return
	...
	var/datum/antagonist/cult/C = the_one_who_called.mind.has_antag_datum(/datum/antagonist/cult,TRUE)
	if(!C.cult_team)
...
/obj/effect/proc_holder/pulse/InterceptClickOn(mob/living/the_one_who_called, params, atom/target)
	if(..())
		return
	if(ranged_ability_user.incapacitated())
		remove_ranged_ability()
	...
	if(!isturf(T))
		return FALSE
	if(target in view(7, get_turf(ranged_ability_user)))
		if((!(iscultist(target) || istype(target, /obj/structure/destructible/cult)) || target == the_one_who_called) && !(attached_action.throwing))
			return
		if(!attached_action.throwing)
			...
			the_one_who_called.update_action_buttons_icon()
			addtimer(CALLBACK(the_one_who_called, TYPE_PROC_REF(/mob,update_action_buttons_icon)), attached_action.base_cooldown)
